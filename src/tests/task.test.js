...
// Limpia la colección para que los tests sean independientes
beforeEach(async () => {
...
describe("Tasks API CRUD", () => {
  it("GET /tasks → should return 200 and an array", async () => { // Verifica que la API devuelva un array de tareas
    const res = await request(app).get("/tasks");
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  }, 10000);

  it("POST /tasks → should create a new task", async () => { // Verifica que se pueda crear una nueva tarea
  ...
  it("PUT /tasks/:id → should update an existing task", async () => { // Verifica que una tarea existente pueda actualizarse
  ...
  it("DELETE /tasks/:id → should delete a task", async () => { // Verifica que una tarea pueda eliminarse
    const task = await Task.create({
